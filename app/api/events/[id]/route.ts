...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const response = await fetch(`${supabase.url}/rest/v1/events?id=eq.${encodeURIComponent(id)}&select=*`, { headers: { apikey: supabase.key, Authorization: `Bearer ${ctx.token}` }, cache: 'no-store' })
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const response = await fetch(`${supabase.url}/rest/v1/events?id=eq.${encodeURIComponent(id)}`, { method: 'PATCH', headers: { apikey: supabase.key, Authorization: `Bearer ${ctx.token}`, 'Content-Type': 'application/json', Prefer: 'return=representation' }, body: JSON.stringify(payload) })
  const data = await response.json().catch(() => ({}))
...
export async function DELETE(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const ctx = await context()
  if (!ctx) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
  const response = await fetch(`${supabase.url}/rest/v1/events?id=eq.${encodeURIComponent(id)}`, { method: 'DELETE', headers: { apikey: supabase.key, Authorization: `Bearer ${ctx.token}`, Prefer: 'return=minimal' } })
