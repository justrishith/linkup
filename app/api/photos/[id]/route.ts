...
export async function GET(_: Request, { params }: { params: { id: string } }) {
  ...
  const response = await fetch(`${supabase.url}/rest/v1/photos?id=eq.${encodeURIComponent(params.id)}&select=storage_path`, { headers: { apikey: supabase.key, Authorization: `Bearer ${token}` }, cache: 'no-store' })
