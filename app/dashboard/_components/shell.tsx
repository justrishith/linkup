...
export default function DashboardShell({ title, eyebrow, children }: { title?: string; eyebrow?: string; children: React.ReactNode }) {
  return <main className="linkup-app min-h-screen bg-[#fafaf8] text-[#111]">
    <header className="sticky top-0 z-20 border-b border-zinc-200 bg-[#fafaf8]/90 backdrop-blur-xl"><div className="mx-auto flex max-w-[1440px] items-center gap-4 px-4 py-3 sm:gap-5 sm:px-6"><Link href="/dashboard" className="mr-2 flex shrink-0 items-center gap-3"><BrandMark size={38}/><div className="text-xl font-black tracking-tight">linkup</div></Link><nav className="hidden flex-1 items-center gap-1 overflow-x-auto sm:flex">{links.map(([href, label]) => <Link key={href} href={href} className="rounded-lg px-3 py-2 text-sm font-bold text-zinc-600 transition hover:bg-white hover:text-black">{label}</Link>)}</nav><ProfileButton /></div></header>
