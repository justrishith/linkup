...
export type PreviewScenario = "ready" | "empty" | "loading" | "error"
...
export default function FullLinkupApp({ preview = false, previewScenario = "ready", initialView = "home", framed = false }: { preview?: boolean; previewScenario?: PreviewScenario; initialView?: CrewView; framed?: boolean }) {
  ...
  const readyPreview = preview && previewScenario === "ready"
  ...
  const load = useCallback(async () => {
    ...
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Unable to load LinkUp") } finally { setLoading(false) }
  }, [groupId, loadGroup])
  useEffect(() => {
    if (preview) return
  ...
  function fail(cause: unknown) { setError(cause instanceof Error ? cause.message : "Something went wrong."); setNotice("") }
  function chooseGroup(id: string) { setGroupId(id); if (!preview) void loadGroup(id) }
  async function createLink(event: FormEvent) {
    event.preventDefault(); if (!linkName.trim()) return
    if (preview) {
      ...
      setGroups((items) => [...items, group]); setGroupId(id); setMembers([{ user_id: localProfile.id, role: "owner", profiles: { display_name: localProfile.display_name } }]); setPlans([]); setMessages([]); setScores([]); setPhotos([]); setLinkName(""); setLinkDescription(""); setShowCreateLink(false); report("Local preview Link created."); return
    }
    setBusy(true)
    try { const result = await request<{ group: { id: string } }>("/api/groups", { method: "POST", body: JSON.stringify({ name: linkName, description: linkDescription }) }); setLinkName(""); setLinkDescription(""); setShowCreateLink(false); setGroupId(result.group.id); await load(); report("Your Link is ready. Invite your people next.") } catch (cause) { fail(cause) } finally { setBusy(false) }
  }
  async function createPlan(event: FormEvent) {
    event.preventDefault()
    const futureIssue = planChoices.some((option) => new Date(option).getTime() <= Date.now()) ? "Choose times in the future." : ""
    if (!groupId || planIssue || futureIssue) { if (planIssue || futureIssue) setError(planIssue || futureIssue); return }
    const options = planChoices.map((starts_at) => ({ starts_at }))
    if (preview) { setPlans((items) => [...items, { id: `local-plan-${Date.now()}`, name: planName.trim(), location: planLocation.trim(), status: "planning", event_date_options: planChoices.map((starts_at, index) => ({ id: `local-option-${Date.now()}-${index}`, starts_at, event_date_votes: [] })) }]); setPlanName(""); setPlanLocation(""); setPlanSlots([{ date: "", time: "18:00" }, { date: "", time: "18:00" }]); setShowCreatePlan(false); report("Local preview plan added."); return }
    setBusy(true)
    try { await request("/api/calendar", { method: "POST", body: JSON.stringify({ groupId, name: planName, location: planLocation, options }) }); setPlanName(""); setPlanLocation(""); setPlanSlots([{ date: "", time: "18:00" }, { date: "", time: "18:00" }]); setShowCreatePlan(false); await loadGroup(groupId); report("Plan added. Everyone can vote now.") } catch (cause) { fail(cause) } finally { setBusy(false) }
  }
  ...
  return <MotionConfig reducedMotion="user"><main className={styles.page} style={framedStyle}>
    ...
    {!active ? <section className={styles.empty}><Sparkles size={30}/><p>Your first Link is a private home for one group of people.</p><button className={styles.primary} onClick={() => setShowCreateLink(true)}><Plus size={17}/> Make your first Link</button></section> : <>
      <section className={styles.content} aria-live="polite"><AnimatePresence initial={false} mode="wait"><motion.div key={view} className={interactions.viewPanel} initial={{ clipPath: "inset(0 100% 0 0 round 22px)" }} animate={{ clipPath: "inset(0 0% 0 0 round 22px)" }} exit={{ clipPath: "inset(0 0 0 100% round 22px)" }} transition={{ duration: 0.22, ease: [0.22, 1, 0.36, 1] }}>
        {view === "home" && <Home activeName={activeName} nextPlan={nextPlan} scores={scores} photos={photos} onPlan={() => setView("plan")} onCreatePlan={() => setShowCreatePlan(true)} onAddMemory={() => photoInputRef.current?.click()}/>}
        {view === "plan" && <Plans plans={plans} members={members} busy={busy} isManager={isManager} onCreate={() => setShowCreatePlan(true)} onVote={castVote} onConfirm={confirmPlan}/>}
        {view === "chat" && <Chat messages={messages} message={message} setMessage={setMessage} onSend={sendMessage} profile={profile} nextPlan={nextPlan} onPlan={() => setView("plan")}/>}
        {view === "links" && <LinksView groups={groups} activeId={groupId} members={members} isManager={isManager} inviteCopied={inviteCopied} onChoose={chooseGroup} onInvite={() => setShowInvite(true)}/>}
        {view === "profile" && <ProfileView profile={profile} scores={scores} busy={busy} onLogout={logout} onEdit={openProfileEditor}/>}
    ...
    {showCreateLink && <Modal title="Make a Link" onClose={() => setShowCreateLink(false)}><form onSubmit={createLink} className={styles.form}><label>Name<input autoFocus value={linkName} onChange={(event) => setLinkName(event.target.value)} placeholder="Rishith’s Weekend"/></label><label>What is this group about?<textarea value={linkDescription} onChange={(event) => setLinkDescription(event.target.value)} placeholder="Plans, boba, and actually leaving the group chat."/></label><button disabled={busy} className={styles.primary}>{busy ? "Making it…" : "Make this Link"}</button></form></Modal>}
    {showCreatePlan && <Modal title="Suggest a plan" onClose={() => setShowCreatePlan(false)}><form onSubmit={createPlan} className={styles.form}><label>What are you doing?<input autoFocus value={planName} onChange={(event) => setPlanName(event.target.value)} placeholder="Late-night boba run"/></label><label>Where?<input value={planLocation} onChange={(event) => setPlanLocation(event.target.value)} placeholder="Cha Time"/></label><div className={styles.slotList}>{planSlots.map((slot, index) => <section key={index} className={styles.planSlot}><div><b>Option {index + 1}</b>{planSlots.length > 2 && <button type="button" onClick={() => setPlanSlots((items) => items.filter((_, itemIndex) => itemIndex !== index))}>Remove</button>}</div><CrewDatePicker value={slot.date} onChange={(date) => setPlanSlots((items) => items.map((item, itemIndex) => itemIndex === index ? { ...item, date } : item))}/><TimeWheel value={slot.time} onChange={(time) => setPlanSlots((items) => items.map((item, itemIndex) => itemIndex === index ? { ...item, time } : item))}/></section>)}</div>{planSlots.length < 4 && <button type="button" className={styles.secondary} onClick={() => setPlanSlots((items) => [...items, { date: "", time: "18:00" }])}><Plus size={15}/> Add another time</button>}{planIssue && <p className={styles.formError} role="status">{planIssue}</p>}<button disabled={busy || Boolean(planIssue)} className={styles.primary}>{busy ? "Saving…" : "Ask the group"}</button></form></Modal>}
...
function Chat({ messages, message, setMessage, onSend, profile, nextPlan, onPlan }: { messages: ChatMessage[]; message: string; setMessage: (value: string) => void; onSend: (event: FormEvent) => void; profile: Profile; nextPlan?: Plan; onPlan: () => void }) {
  return <section className={styles.chat}><div className={styles.titleRow}><div><p>THE GROUP CHAT</p><h1>Say it once.</h1></div><MessageCircle size={25}/></div><DecisionHandoff title={nextPlan ? nextPlan.name : "Start the next plan"} detail={nextPlan ? "See the choices and make the call." : "Put two times in front of the group."} onOpen={onPlan}/><div className={styles.messages}>{messages.length ? messages.map((item) => <article key={item.id} className={item.user_id === profile.id ? styles.yours : ""}><i>{initial(item.profiles?.display_name)}</i><div><b>{item.user_id === profile.id ? "You" : item.profiles?.display_name || "Link member"}</b><p>{item.body}</p></div></article>) : <p className={styles.muted}>No messages yet. Break the silence.</p>}</div><form className={styles.composer} onSubmit={onSend}><input value={message} maxLength={2000} onChange={(event) => setMessage(event.target.value)} placeholder="Message your people"/><button disabled={!message.trim()} aria-label="Send message"><Send size={17}/></button></form></section>
}

function LinksView({ groups, activeId, members, isManager, inviteCopied, onChoose, onInvite }: { groups: Group[]; activeId: string; members: Member[]; isManager: boolean; inviteCopied: boolean; onChoose: (id: string) => void; onInvite: () => void }) {
  return <section><div className={styles.titleRow}><div><p>YOUR PEOPLE</p><h1>Your Links.</h1></div><button className={styles.circleButton} onClick={onInvite} aria-label="Invite someone"><Plus size={20}/></button></div>{inviteCopied && <aside className={interactions.inviteReceipt} role="status"><Copy size={16}/><div><b>Private invite copied</b><span>Now send it where your crew already talks.</span></div></aside>}<div className={styles.linkCards}>{groups.map((group) => <button key={group.group_id} className={group.group_id === activeId ? styles.selectedLink : ""} onClick={() => onChoose(group.group_id)}><i>{initial(group.groups?.name)}</i><span><b>{group.groups?.name || "Untitled Link"}</b><small>{group.member_count || 0} people · {group.groups?.visibility || "private"}</small></span><ChevronRight size={18}/></button>)}</div><section className={styles.members}><div className={styles.sectionHead}><span>MEMBERS</span><Users size={17}/></div>{members.length ? members.map((member) => <div key={member.user_id}><Avatar name={member.profiles?.display_name} url={member.profiles?.avatarUrl || member.profiles?.avatar_url}/><b>{member.profiles?.display_name || "Link member"}</b><small>{member.role}</small></div>) : <p className={styles.muted}>Members appear here after your first invite is accepted.</p>}{isManager && <button className={styles.secondary} onClick={onInvite}><Copy size={15}/> Invite another person</button>}</section></section>
}

function ProfileView({ profile, scores, busy, onLogout, onEdit }: { profile: Profile; scores: Score[]; busy: boolean; onLogout: () => void; onEdit: () => void }) {
  ...
  return <section><div className={styles.profile}><Avatar name={profile.display_name} url={profile.avatarUrl || profile.avatar_url}/><div><p>YOUR SPACE</p><h1>{profile.display_name || "Your LinkUp"}</h1><span>{profile.email}</span></div><button className={styles.profileEdit} onClick={onEdit}>Edit</button></div><section className={interactions.showingUp} aria-label="Showing-up points"><div><span>SHOWING UP</span><b>{points} points</b></div><div className={interactions.showingUpTrack} aria-hidden="true"><span style={{ width: `${progress}%` }}/></div><details><summary>What counts?</summary><p>Verified plans reward everyone fairly. This is your current LinkUp total, not a streak.</p></details></section><div className={styles.settings}><div><b>Notifications</b><span>Plans and replies from your Links</span></div><div><b>Privacy</b><span>Photos are private to Link members</span></div></div><button className={styles.logout} disabled={busy} onClick={onLogout}><LogOut size={16}/> Log out</button></section>
}
...
function Modal({ title, children, onClose }: { title: string; children: React.ReactNode; onClose: () => void }) {
  return <div className={styles.modalBackdrop} role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className={styles.modal} role="dialog" aria-modal="true" aria-labelledby="modal-title"><button className={styles.close} onClick={onClose} aria-label="Close">×</button><h2 id="modal-title">{title}</h2>{children}</section></div>
}
